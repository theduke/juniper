    ...
    for<'b> &'b S: ScalarRefValue<'b>,
{
    ...
    where
        ...
        S: Send + Sync,
    {
        Ok(value.resolve_async(info, self.current_selection_set, self).await)
    }
    ...
    for<'b> &'b S: ScalarRefValue<'b>,
{
...

    {
        ...
        value = match op.item.operation_type {
            OperationType::Query => executor.resolve_into_value_async(&root_node.query_info, &root_node).await,
            OperationType::Mutation => {
                executor.resolve_into_value_async(&root_node.mutation_info, &root_node.mutation_type).await
            }
        };
    }
