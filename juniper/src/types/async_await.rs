...
use super::base::{GraphQLType, is_excluded, merge_key_into, Arguments, resolve_selection_set_into};
    ...
    for<'b> &'b S: ScalarRefValue<'b>,
{
    ...
    let mut async_values = FuturesUnordered::<BoxFuture<'a, AsyncValue<S>>>::new();
        ...
        match *selection {
            Selection::Field(Spanning {
                ...
                let field_future = async move {
                    ...
                    // two-level boxing.
                    let res = instance.resolve_field_async(
                        info,
                        f.name.item,
                        &args,
                        &sub_exec,
    ...
    while let Some(item) = async_values.next().await {
        match item {
            ...
            AsyncValue::Nested(obj) => {
                match obj {
                    v @ Value::Null => { return v; },
                    Value::Object(obj) => {
                        for (k, v) in obj {
                            merge_key_into(&mut object, &k, v);
                        }
